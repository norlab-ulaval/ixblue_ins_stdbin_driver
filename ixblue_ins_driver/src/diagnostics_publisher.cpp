...
DiagnosticsPublisher::DiagnosticsPublisher(rclcpp::Node::SharedPtr nh):
    nh(nh),
    diagnosticsUpdater(nh)
{
    nh->declare_parameter<double>("expected_frequency", 10.0);
    nh->get_parameter("expected_frequency", expected_frequency);
    nh->declare_parameter<double>("max_latency", 1.0);
    nh->get_parameter("max_latency", max_latency);
    nh->declare_parameter<double>("connection_lost_timeout", 10.0);
    nh->get_parameter("connection_lost_timeout", connection_lost_timeout);

    RCLCPP_INFO(nh->get_logger(), "Expected frequency for diagnostics : %.2f Hz", expected_frequency);
    RCLCPP_INFO(nh->get_logger(), "Max latency acceptable for diagnostics : %.3f s", max_latency);
    RCLCPP_INFO(nh->get_logger(), "Connection lost timeout : %.3f s", connection_lost_timeout);
    ...
    diagnosticsTimer = nh->create_wall_timer(std::chrono::milliseconds(100),
...
void DiagnosticsPublisher::updateStatus(
    ...
    const boost::optional<ixblue_stdbin_decoder::Data::INSAlgorithmStatus>&
        algorithmStatus)
{
    lastMessageReceivedStamp = nh->get_clock()->now();
    ...
    diagnostic_updater::DiagnosticStatusWrapper& status)
{
    ...
    else if((nh->get_clock()->now() - lastMessageReceivedStamp).seconds() >
